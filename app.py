...
# ¡Actualizada con las mínimas de 400m!
MINIMAS_DB = {
    "Hombre": {
...
def obtener_coeficiente(distancia, genero):
    if distancia == "60m":
        ...
        return 0.0  # El viento no afecta de forma neta/oficial en los 400m (vuelta completa)
    return 0.0
...
with tab1:
    ...
    with col2:
        # Si elige 400m, el viento es irrelevante reglamentariamente
        if distancia == "400m":
            viento = st.number_input("Viento medido (m/s):", value=0.0, step=0.1, disabled=True, help="En los 400m no se mide el viento reglamentariamente.", key="ind_wind")
        else:
        ...
        if distancia is None or tiempo_real is None or viento is None or genero is None or categoria_elegida is None:
            ...
            if categoria_elegida in MINIMAS_DB[genero].get(distancia, {}):
                ...
                if distancia == "400m" and categoria_elegida == "Sub16":
                    st.info(f"ℹ️ Curiosidad: La RFEA no contempla los 400m en Sub16. En esta categoría la prueba oficial de la換en de pista es el **300m lisos**.")
                else:
            ...
            if distancia == "60m":
                ...
                f_min_400, f_max_400 = (2.15, 2.25) if genero == "Hombre" else (2.20, 2.30) # Añadida proyección a 400m
                proyecciones_rango["100m"] = (tiempo_neutral / f_min_100, tiempo_neutral / f_max_100)
                proyecciones_rango["400m"] = (tiempo_neutral * f_min_400, tiempo_neutral * f_max_400)

            elif distancia == "400m":
                # Proyección inversa desde el 400m hacia el 200m
            ...
            if distancia == "400m":
                st.write("Nota: Al ser una prueba de 400m, las variaciones de viento lineal en recta no alteran el cómputo oficial:")
                vientos_simular = [0.0]
            else:
...
with tab2:
    ...
    st.write("¿Quién ha sido realmente más rápido? Compara a dos atletas que corrieron en condiciones o series diferentes.")
    ...
    with col_at1:
        ...
        if duelo_distancia == "400m":
            v1 = st.number_input("Viento medido (m/s):", value=0.0, step=0.1, disabled=True, key="v1", help="No aplica en 400m")
        else:
    ...
    with col_at2:
        ...
        if duelo_distancia == "400m":
            v2 = st.number_input("Viento medido (m/s):", value=0.0, step=0.1, disabled=True, key="v2", help="No aplica en 400m")
        else:
    ...
    if st.button("🔥 ¡Iniciar Duelo Virtual!", type="primary"):
        if t1 is None or v1 is None or t2 is None or v2 is None:
            ...
            st.write(f"👉 El tiempo de **{nom1}** equivale a: **{n1:.2f}s**")
            st.write(f"👉 El tiempo de **{nom2}** equivale a: **{n2:.2f}s**")
            
            if abs(n1 - n2) < 0.001:
                st.info(f"🤝 **¡Empate técnico absoluto!** Ambos han clavado la misma marca.")
            elif n1 < n2:
                diferencia = n2 - n1
                st.success(f"👑 **¡Ganador Virtual: {nom1}!** Ha aventajado a {nom2} por **{diferencia:.2f}s**.")
            else:
                diferencia = n1 - n2
                st.success(f"👑 **¡Ganador Virtual: {nom2}!** Ha aventajado a {nom1} por **{diferencia:.2f}s**.")
